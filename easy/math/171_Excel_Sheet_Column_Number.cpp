//
// Created by ranpeng on 2020/4/28.
//

#include "common.h"

/*
Given a column title as appear in an Excel sheet, return its corresponding column number.

For example:

    A -> 1
    B -> 2
    C -> 3
    ...
    Z -> 26
    AA -> 27
    AB -> 28
    ...
Example 1:

Input: "A"
Output: 1
Example 2:

Input: "AB"
Output: 28
Example 3:

Input: "ZY"
Output: 701
*/

class Solution {
public:
    int titleToNumber(string s) {
        int res = 0;
        for (int i = 0; i < s.length(); ++i) {
            res += (s[i]-'A'+1) * std::pow(26, s.length()-i-1);
        }
        return res;
    }
};

int main() {
    std::cout << Solution().titleToNumber("AZ") << std::endl;
    std::cout << Solution().titleToNumber("BA") << std::endl;
    std::cout << Solution().titleToNumber("AA") << std::endl;
    std::cout << Solution().titleToNumber("BA") << std::endl;
    std::cout << Solution().titleToNumber("Y") << std::endl;
    std::cout << Solution().titleToNumber("Z") << std::endl;
    std::cout << Solution().titleToNumber("YZ") << std::endl;
    std::cout << Solution().titleToNumber("ZA") << std::endl;
    std::cout << Solution().titleToNumber("ZY") << std::endl;
    return 0;
}
